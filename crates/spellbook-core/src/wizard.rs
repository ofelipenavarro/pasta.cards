...
/// The new-deck dialog's auto-build request. Kept as a struct because its
/// serde defaults are the rules: no bracket means bracket 3, no mode means
/// "suggest".
...
/// The finished build: the deck that was written, and the summary the new-deck
/// dialog shows before it navigates into it.
...
/// What one auto-build produced.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct BuildMeta {
    pub commander: String,
    pub color_identity: String,
    pub bracket: i64,
    /// "suggest" (full pool) or "owned" (collection only).
    pub mode: String,
    pub nonland_count: usize,
    pub land_count: i64,
    pub missing_count: usize,
    pub borrowed_count: usize,
    /// Names of the chosen cards the user doesn't own at all - the shopping list.
    pub missing: Vec<String>,
    /// Chosen cards whose only copies are sleeved in another deck.
    pub borrowed: Vec<BorrowedCard>,
}

/// A chosen card the user owns, but only as a copy already in another deck.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct BorrowedCard {
    pub name: String,
    pub deck: Option<String>,
}

/// Where a deck's copies of one card actually are.
///
/// This was a free-form string in the payload the deck view read. The four
/// values are a rule that both this module and the deck view branch on, so
/// they are a type.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum OwnershipStatus {
    /// This deck holds its own copies - nothing to resolve.
    OwnedHere,
    /// Short a copy, but there is a free one in the collection to sleeve in.
    OwnedFree,
    /// Short a copy, and the only ones owned are in other decks.
    OwnedInDeck,
    /// Not in the collection at all.
    Missing,
}

/// One deck holding copies of a card, for the "usar aqui significa desmontar" note.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeckHolding {
    pub deck: String,
    pub copies: i64,
}

/// One card's ownership as seen from one deck.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct CardOwnership {
    pub status: OwnershipStatus,
    /// Copies backing the status: the deck's own copies for `OwnedHere`, the
    /// free ones for `OwnedFree`, 0 for the two statuses with nothing to draw on.
    pub copies: i64,
    /// Copies still missing from this deck. 0 for `OwnedHere`.
    pub short: i64,
    /// First entry of `decks`, which is the one the tag names.
    pub deck: Option<String>,
    pub decks: Vec<DeckHolding>,
}
        ...
        match deck_id {
            ...
            Some(id) => {
                let slot = e
                    .by_deck
                    .entry(id)
                    .or_insert_with(|| (deck_name.unwrap_or_default(), 0));
                slot.1 += quantity;
            }
        }
    }
...
pub fn build(p: &AutoBuildIn) -> Result<BuildOutcome> {
    let owned_only = p.mode.as_deref() == Some("owned");
    let cdb = open_cards_db().ok_or_else(|| {
        Error::BadRequest(
            "Base de cartas ainda não construída — atualize a base de dados primeiro.".into(),
        )
    })?;
    let con = open_app_db()?;
    ...
    let commander: (String, String, String) = cdb
        .query_row(
            ...
            |r| {
                Ok((
                    r.get::<_, String>("name")?,
                    r.get::<_, Option<String>>("color_identity")?
                        .unwrap_or_default(),
        ...
        .map_err(|_| {
            Error::BadRequest(format!(
                "Comandante não encontrado na base local: {}",
                p.commander_name
            ))
        })?;
    let (commander_name, commander_ci, ctype) = commander;
    if !ctype.contains("Legendary")
        || !(ctype.contains("Creature") || ctype.contains("Planeswalker"))
    {
        return Err(Error::BadRequest(format!(
            "'{commander_name}' não é uma carta lendária elegível para comandante."
        )));
    }
    ...
    let mut pool: Vec<Candidate> = Vec::new();
    {
        let mut stmt = cdb.prepare(&format!(
            "SELECT {CARD_COLS} FROM cards WHERE commander_legal = 'legal'"
        ))?;
        let rows = stmt.query_map([], |r| {
            Ok((
                ...
                r.get::<_, Option<String>>("color_identity")?
                    .unwrap_or_default(),
                r.get::<_, Option<String>>("type_line")?.unwrap_or_default(),
                r.get::<_, Option<String>>("oracle_text")?
                    .unwrap_or_default(),
    ...
    pool.sort_by(|a, b| {
        rank_of(a)
            .cmp(&rank_of(b))
            .then(a.edhrec_rank.cmp(&b.edhrec_rank))
    });
    ...
    con.execute(
        ...
        params![
            ...
            p.philosophy.clone().unwrap_or_else(|| {
                let m = if owned_only {
                    "apenas cartas da coleção"
                } else {
                    "sugerindo cartas fora da coleção"
                };
                format!(
                    "Montado automaticamente (bracket {}, {m}) a partir da base local do Scryfall.",
                    p.bracket
                )
            })
        ],
    ...
    let borrowed: Vec<&Candidate> = chosen
        .iter()
        .filter(|c| c.ownership == "owned_in_deck")
        .collect();
    ...
    let meta = BuildMeta {
        ...
        missing: missing.iter().map(|c| c.name.clone()).collect(),
        borrowed: borrowed
            .iter()
            .map(|c| BorrowedCard {
                name: c.name.clone(),
                deck: c.owned_in_deck.clone(),
            })
            .collect(),
    };
...
fn build_manabase(ci: &str, chosen: &[Candidate], land_count: usize) -> Vec<(String, i64)> {
    ...
    for card in chosen {
        for c in &colors {
            pips.entry(*c)
                .and_modify(|n| *n += card.mana_cost.matches(&format!("{{{c}}}")).count());
        }
    }
...
pub fn deck_ownership(con: &Connection, deck_id: i64) -> HashMap<String, CardOwnership> {
    let owned = collection_index(con);
    ...
    for (name, needed) in rows.flatten() {
        ...
        let v = if short <= 0 {
            CardOwnership {
                status: OwnershipStatus::OwnedHere,
                copies: here,
                short: 0,
                deck: None,
                decks: Vec::new(),
            ...
            CardOwnership {
                status: OwnershipStatus::OwnedFree,
                copies: copies.free,
                short,
                deck: None,
                decks: Vec::new(),
            ...
            CardOwnership {
                status: OwnershipStatus::OwnedInDeck,
                copies: 0,
                short,
            ...
            CardOwnership {
                status: OwnershipStatus::Missing,
                copies: 0,
                short,
                deck: None,
                decks: Vec::new(),
