...
//!
//! Lives apart from `db.rs` (which owns app.db) and `ops/cards.rs` (which owns the index)
...
use crate::ops::cards::canonical_name;
...
pub fn canonicalise_card_names() -> usize {
    ...
    for table in ["collection", "deck_cards"] {
        ...
        for raw in names {
            let Some(canon) = canonical_name(&cdb, &raw) else {
                continue;
            };
            ...
            let n = con
                .execute(
                    &format!(
                        "UPDATE {table} SET card_name = ?1 WHERE card_name = ?2 COLLATE NOCASE"
                    ),
